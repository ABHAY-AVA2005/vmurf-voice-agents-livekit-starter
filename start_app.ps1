...
# Check for uv at absolute path or PATH
$uvPath = "C:\Users\Admin\.local\bin\uv.exe"
if (-not (Test-Path $uvPath) -and -not (Test-CommandExists "uv")) {
...
$uvCommand = if (Test-Path $uvPath) { $uvPath } else { "uv" }
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$repoRoot\backend'; & '$uvCommand' run python src/agent.py dev"
